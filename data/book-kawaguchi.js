...
const MATH_UNITS_G3_KAWA=(()=>{
  ...
  return [
    ...
    /* 東書の「大きい数」は一億まで → u5（一万をこえる数）の3問型に
       十万〜一億の階段（10000のまとまり・7けたのよみ書き・10ばい/10でわる・一億）を足した拡張版。
       7けた以上は直打ちがつらいので4択にする。bun（文章問題）と⚡おうようは u5 を共有 */
    c("u5", {id:"kwu9",  name:"大きい数のしくみ",        term:2, s:[9,15], gen(){
      const v=rnd(1,6);
      ...
      if(v===3){ const n=rnd(12,98); return q("10000が "+n+"こで いくつ？", n*10000, "big"); }
      if(v===4){ const n=rnd(2,9);
        return qc(n+"000万 を 数字で 書くと？", String(n*10000000),
          [String(n*10000000), String(n*1000000), String(n*100000000), String(n*100000)], "big"); }
      if(v===5){ const n=rnd(11,98), c4=[(n*10)+"万", n+"万", (n*100)+"万", (n*1000)+"万"];
        if(Math.random()<0.5) return qc(n+"万 を 10ばい した かずは？", (n*10)+"万", c4, "big");
        return qc((n*10)+"万 を 10で わった かずは？", n+"万", c4, "big"); }
      return qc("1000万を 10こ あつめた かずを なんと いう？", "一億", ["一億","一千万","十億","百万"], "big"); }}),
